...
public sealed class GameUIRoot  : MonoBehaviour
{
    ...
    private void Awake()
    {
        bool IsWeaponProvider = InitializePlayerWeaponProvider();
        if (!IsWeaponProvider)
        {
            return;
        }

        bool IsVehicleProvider = InitializePlayerVehicleProvider();
        if (!IsVehicleProvider)
        {
            return;
        }
    }
    ...
    private bool InitializePlayerVehicleProvider()
    {
        ...
        var bus = CompositionRoot.Instance.Events;
        _healthVm = new HealthViewModel(provider.GetComponent<ITakesDamage>(), bus);
    ...
    private bool InitializePlayerWeaponProvider()
    {
        ...
        {
            _weaponSlotVms[i] = new WeaponSlotViewModel(cfgs[i], weapons[i], CompositionRoot.Instance.Events);
