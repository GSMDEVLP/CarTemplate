...
public class CameraImpulse : MonoBehaviour
{
    ...
    if (owner != _player) return;

        if (e.Kind != null && (e.Kind == WeaponKind.Homing || e.Kind == WeaponKind.Straight))
            Shake(_rocketKick);
    }
    ...
    private void OnExplosion(Explosion e)
    {
        ...
        float dist = Vector3.Distance(_player.transform.position, UnityVectorAdapter.ToUnity(e.Position));
    ...
    private bool IsPlayerTarget(EntityId target)
    {
        var pid = GetPlayerId();
        return pid.IsValid && target.Equals(pid);
    }
}
